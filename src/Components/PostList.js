...
import { getRecentPosts } from '../actions/actionCreators';
import SearchBar from './SearchBar';
import './PostList.css';

function PostList({ upVote, downVote }) {
    ...
    return (
        <div className="row justify-content-around my-5" data-testid="PostList">
            ...
            {postsArr.length > 0 ?
                postsArr.map(p => (
                    < div key={p.post_id} className="col-5 p-0 mb-4 bg-light PostList" >
                        <div className="container mt-1">
                            <Link className="text-decoration-none" to={`/post/${p.post_id}`}>
                                <h3>{p.title}</h3>
                            </Link>
                            <div className="d-flex flex-row justify-content-center mb-2">
                                <div onClick={() => upVote(p.blog_id, "blogs")}>
                                ...
                                <div onClick={() => downVote(p.blog_id, "blogs")}>
                            ...
                            <div className="d-flex justify-content-between">
                                <small className="mb-1">From: <Link to={`/blog/${p.blog_id}`}>{p.blog_title}</Link></small>
                                <small className="mb-1"> By: <Link to={`/user/${p.username}`}>{p.username}</Link></small>
                            </div>
                        </div>
                    </div>
                )) :
